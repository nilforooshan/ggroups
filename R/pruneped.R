#' @title Pedigree prunning
#'
#' @description Prunning pedigree in two different modes (strict, loose)
#'
#' @details
#' In strict pruning, animals without progeny and phenotype are recursively deleted from the pedigree. Therefore, all uninfluential animals are deleted. The downside is that animals without phenotype or phenotyped progeny cannot receive any genetic merit based on the information from their phenotyped relatives.
...
pruneped = function(ped, pheno, mode) {
   ...
   if(mode=="strict")
   {
      print("Discarding animals without progeny and phenotype")
      newped = data.frame()
      oldped = ped
      iter = 0
      while(nrow(newped) < nrow(oldped))
      {
         ...
         print(paste(nrow(newped), "Animals in the pedigree"))
         oldped = newped
      ...
      print("Discarding animals with no parent and progeny")
      parents = c(unique(newped$SIRE), unique(newped$DAM))
      ...
      print(paste(nrow(ped), "Animals in the pedigree"))
